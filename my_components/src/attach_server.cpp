...
AttachServer::AttachServer(const rclcpp::NodeOptions &options)
    : Node("attach_service_node", options), y_{0.0},
  ...
  this->timer_tf_broadcaster = this->create_wall_timer(
      ...
      std::bind(&AttachServer::publish_cart_frame, this), callback_g1);
...
void AttachServer::subscriber_scan_callback(
...
void AttachServer::subscriber_odom_callback(
...
void AttachServer::publish_cart_frame() {
  ...
  std::string f_ref = "odom";
  std::string f_tar = "robot_front_laser_base_link";
  try {
    odom2laser_msg =
  ...
  odom2cart_msg.header.frame_id = "odom";
  ...
  // node feedback
  RCLCPP_INFO_ONCE(this->get_logger(), "Publishing `odom` to `cart_frame`");

  // publish transformation
  this->tf_dynamic_broadcaster->sendTransform(odom2cart_msg);
}

void AttachServer::move_robot_to_cart() {
  // node feedback
  RCLCPP_INFO_ONCE(this->get_logger(), "Moving robot towards the shelf");

  // parameters
  std::string f_ref = "robot_base_link";
...
void AttachServer::move_robot_under_cart() {
  // node feedback
